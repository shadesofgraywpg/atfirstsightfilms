...
class stag_twitter_feeds extends WP_Widget{
  ...
  function widget($args, $instance){
    ...
    if(empty($instance['consumer_key']) || empty($instance['consumer_secret']) || empty($instance['access_token']) || empty($instance['access_token_secret']) || empty($instance['cache_time']) || empty($instance['twitter_username'])){
      echo '<strong>Please fill all widget settings!</strong>' . $after_widget;
      return;
    }

    $last_cache = stag_get_option('twitter_widget_last_cache_time');
    ...
    if( $diff >= $crt || empty($last_cache) ){
      ...
      $tweets = $connection->get("https://api.twitter.com/1.1/statuses/user_timeline.json?screen_name=".$instance['twitter_username']."&count=10&exclude_replies=true") or die('Couldn\'t retrieve tweets! Wrong username?');
      ...
      update_option('twitter_widget_tweets', serialize($tweets_array));
    ...
    $widget_tweets = maybe_unserialize(get_option('twitter_widget_tweets'));

    if(!empty($widget_tweets)){
      echo '
        <div class="stag-twitter-feeds">
          <ul>';
          $count = 1;
          $protocol = is_ssl() ? 'https:' : 'http:';
          foreach($widget_tweets as $tweet){
            echo '<li><span>'. twitter_widget_convert_links($tweet['text']) .'</span><div class="time"><a href="'.$protocol.'//twitter.com/'.$instance['twitter_username'].'/statuses/'.$tweet['status_id'].'" target="_blank">'.twitter_widget_relative_time($tweet['created_at']).'</a></div></li>';
            if( $count == $instance['postcount']){ break; }
            $count++;
          }
      echo '</ul></div>';
      if( !empty($tweettext) ) {
        echo '<a href="'.$protocol.'//twitter.com/'.$twitter_username.'" class="twitter-link">'.$tweettext.'</a>';
      }
    }
  ...
  function form($instance){
    ...
    <p>
      <label for="<?php echo $this->get_field_id('consumer_key'); ?>"><?php _e('Consumer Key:', 'stag'); ?></label>
    ...
    <p>
      <label for="<?php echo $this->get_field_id('consumer_secret'); ?>"><?php _e('Consumer Secret:', 'stag'); ?></label>
    ...
    <p>
      <label for="<?php echo $this->get_field_id('access_token'); ?>"><?php _e('Access Token:', 'stag'); ?></label>
    ...
    <p>
      <label for="<?php echo $this->get_field_id('access_token_secret'); ?>"><?php _e('Access Token Secret:', 'stag'); ?></label>
    ...
    <p><span><?php _e('Details like Consumer key and secret can be obtained from <a href="//dev.twitter.com/apps" target="_blank">twitter</a>', 'stag'); ?></span></p>
